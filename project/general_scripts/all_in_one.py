...
### ALLOW USER FOR WINDOWLEN / CROSVAL INPUT ###
windowlen = 0
while windowlen % 2 == 0:
    windowlen = int(input("select window len(has to be odd number):"))

crosval = int(input("select fold of cross validation:"))
### CREATING LIST OF ALL WINDOWS AND ALL TOPOLOGY FEATURES WITH SAME INDEXES###
n = windowlen // 2
listofwindows = []
listofstates = []
...
clf = svm.SVC()
score = cross_val_score(clf, encodedwindows, listalls, cv=crosval, verbose=True)
